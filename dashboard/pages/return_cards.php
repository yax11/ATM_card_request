...
// DELETE FROM `cards` WHERE `account_number` = '513223 1895577486'
$q2 = "SELECT * FROM `card1` WHERE `account_number` = '$customer' ";

$query2 = $conn->query($q2);

if ($query2->num_rows == 1) {
  $card1 = $query2->fetch_assoc();



    $number = $card1["card_number"];
    $status = $card1["card_status"];
    $exp = $card1["card_exp"];
...
c;
              $view = <<<c
            <a class="btn bg-gradient-light mb-0" onclick="track_card()"><i class="fas"></i>View</a>
c;
            // code...
            break;
...
$cards1 = <<<CARDS
                      ...
                      <input type="hidden" name="card1" id="card1" value="$number">
                      <input type="hidden" name="card1_customer" id="card1" value="$number">
                      $view
                      </td>
                    </tr>
